...
install_files() {
  ...
  rm --force -- /etc/whonix_firewall.d/*.conf
...
cleanup_markers() {
  rm --force -- /usr/share/anon-gw-base-files/gateway
  rm --force -- /usr/share/anon-ws-base-files/workstation
  rm --force -- /usr/share/libvirt-dist/marker
  rm --recursive --force -- /run/sdwdate
  rm --force -- /run/whonix_firewall/first_run_current_boot.status
  rm --force -- /run/whonix_firewall/consecutive_run.status
}
...
clear_config() {
  rm --force -- /etc/whonix_firewall.d/50_test.conf
}
